    ...
    if (world_rank == 0) {
      ...
      for (int i = 1; i < world_size; ++i) {
        ...
        for (int j = start; j < end; ++j) {
          ...
          MPI_Send(all_entries[j].c_str(), len, MPI_CHAR, i, 0, MPI_COMM_WORLD);
        }

        int b_len = borough.size();
        MPI_Send(&b_len, 1, MPI_INT, i, 0, MPI_COMM_WORLD);
        MPI_Send(borough.c_str(), b_len, MPI_CHAR, i, 0, MPI_COMM_WORLD);
      }
...
int main(int argc, char** argv) {
  ...
  if (world_rank == 0) {
    ...
    while (true) {
      ...
      int b_len;
      MPI_Recv(&b_len, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      std::vector<char> borough_buffer(b_len);
      MPI_Recv(borough_buffer.data(), b_len, MPI_CHAR, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
